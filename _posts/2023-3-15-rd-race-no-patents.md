...
First, we explore how the market evolves over time. Assume $$n$$ firms enter the R&D race. Firms will discover innovations and enter the industry at different points in time. Let $$\tau_1, \tau_2, ..., \tau_n$$ be the times at which firms $$1, 2, ..., n$$ innovate, and let $$\tau_{(1)}, \tau_{(2)},...,\tau_{(n)}$$ be the arrival times of the first, second, and $$nth$$ innovation. In order to understand how the composition of the market will change over time, we must understand the distribution of these $$\tau_{(i)}$$.
